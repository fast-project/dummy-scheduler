...
enum messageType { STARTVM, STOPVM, MIGRATEVM, INITAGENT, STOPMONITOR };
...
class startvm : public message {
  public:
	startvm(name hostname, std::vector<machineConf> &vm_conf, std::shared_ptr<fast::MQTT_communicator> comm, int Qos)
		: message(std::string("fast/migfra/") + hostname + std::string("/task"), comm, "scheduler", STARTVM, Qos),
		  vm_configurations(vm_conf), hostname(hostname) {
		this->send();
	}
  ...
  public:
	...
	stopvm(name host, machines vmMachines, std::shared_ptr<fast::MQTT_communicator> comm, int Qos)
		: hostname(host), vmMachines(vmMachines),
		  message(std::string("fast/migfra/") + host + std::string("/task"), comm, "scheduler", STARTVM, Qos) {
		this->send();
	}
...
class migratevm : public message {
  public:
	migratevm(name host, name vm_name, name destination, parameter par, std::shared_ptr<fast::MQTT_communicator> comm,
			  int Qos)
  ...
  public:
	...
	initAgent(name host, parameter categories, name repeat, std::shared_ptr<fast::MQTT_communicator> comm, int Qos)
		: host(host), categories(categories), repeat(repeat),
		  message(std::string("fast/agent/") + host + std::string("/task"), comm, "scheduler", INITAGENT, Qos) {
		std::cout << "XXX: " << std::string("fast/agent/") + host + std::string("/task") << std::endl;
		this->send();
	};
	// Override these two methods to state which members should be serialized
	YAML::Node emit() const override;
  ...
  public:
	...
	stopMonitor(name host, name job_id, name process_id, std::shared_ptr<fast::MQTT_communicator> comm, int Qos)
		: host(host), job_id(job_id), process_id(process_id),
		  message(std::string("fast/agent/") + host + std::string("/task"), comm, "scheduler", STOPMONITOR, Qos) {
		this->send();
	};
	// Override these two methods to state which members should be serialized
	YAML::Node emit() const override;
